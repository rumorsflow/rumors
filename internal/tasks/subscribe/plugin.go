...
func (p *Plugin) ProcessTask(ctx context.Context, task *asynq.Task) error {
	...
	broadcast := lo.Map(items, func(item models.Feed, _ int) string {
		return item.Id
	})
	if room.Broadcast != nil {
		broadcast = append(broadcast, *room.Broadcast...)
	}
	broadcast = lo.Uniq(broadcast)
	...
	p.sender.SendView(message.Chat.ID, tgbotsender.ViewSuccess, fmt.Sprintf(consts.SuccessMsgSubscribed, message.CommandArguments()))
