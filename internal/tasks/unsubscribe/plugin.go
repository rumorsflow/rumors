...
func (p *Plugin) ProcessTask(ctx context.Context, task *asynq.Task) error {
	...
	if err := p.service.DeleteFromBroadcastByHost(ctx, message.Chat.ID, message.CommandArguments()); err != nil {
	...
	p.sender.SendView(message.Chat.ID, tgbotsender.ViewSuccess, consts.SuccessMsgUnsubscribed)
