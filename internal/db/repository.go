...
func NewRepository[T repository.Entity](database *mongodb.Database, collection string, options ...Option[T]) (*Repository[T], error) {
	if database == nil {
		return nil, fmt.Errorf("%s %w", repository.OpNew, ErrMissingDB)
	}
	if collection == "" {
		return nil, fmt.Errorf("%s %w", repository.OpNew, ErrMissingCollection)
	}
	...
	for _, option := range options {
		if err := option(r); err != nil {
			return nil, fmt.Errorf("%s while applying option, %w", repository.OpNew, err)
		}
	}
...
func (r *Repository[T]) Count(ctx context.Context, filter any) (count int64, err error) {
	...
	if err != nil {
		err = fmt.Errorf("%s %w", repository.OpCount, err)
	}
	return
}
...
func (r *Repository[T]) Find(ctx context.Context, criteria *repository.Criteria) ([]T, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%s %w", repository.OpFind, err)
	}
...
func (r *Repository[T]) FindIter(ctx context.Context, criteria *repository.Criteria) (repository.Iter[T], error) {
	if r.entityFactory == nil {
		return nil, fmt.Errorf("%s %w", repository.OpFindIter, repository.ErrMissingEntityFactory)
	}
...
func (r *Repository[T]) FindByID(ctx context.Context, id uuid.UUID) (value T, err error) {
	if r.entityFactory == nil {
		return value, fmt.Errorf("%s %w", repository.OpFindByID, repository.ErrMissingEntityFactory)
	}
...
func (r *Repository[T]) Save(ctx context.Context, entity T) (err error) {
	id := entity.EntityID()
	if id == uuid.Nil {
		return fmt.Errorf("%s %w", repository.OpSave, repository.ErrMissingEntityID)
	}
...
func repoErr(op string, err error, id uuid.UUID) error {
	...
	return fmt.Errorf("%s %v -> %w", op, id, err)
}
