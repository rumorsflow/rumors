...
func (i *Iterator[T]) Close(ctx context.Context) (err error) {
	...
	if i.decodeErr != nil || err != nil {
		err = fmt.Errorf("%s %w. decode: %w", OpIter, err, i.decodeErr)
	}
...
func (i *Iterator[T]) valid() bool {
	...
	if i.Cursor == nil {
		i.decodeErr = fmt.Errorf("%s %w", OpFind, ErrMissingCursor)
		return false
	}

	if i.Factory == nil {
		i.decodeErr = fmt.Errorf("%s %w", OpFind, ErrMissingEntityFactory)
		return false
	}
